  ...
  assemblingElements() {
    ...
    for (let i = 0; i < commandMenus.length; i++) {
      ...
      for (let j = 0; j < commandMenus[i].length; j++) {
        commandMenus[i][j] = new EquipmentItemNameView(this.context.memberNameContexts[i][j], this.context.commandMenus[i][j].commandName, this.frameCanvases, this.charCanvases);
